...
import { useSpacedRepetition, useActiveRecall } from '@/lib/hooks/useSpacedRepetition'
...
export default function SpacedRepetitionPage() {
  ...
  const [reviewingNote, setReviewingNote] = useState<typeof notes[0] | null>(null)
  ...
  const { isRevealed, timeSpent, startRecall, revealAnswer, reset } = useActiveRecall()
  ...
  const handleCreateNote = async (e: React.FormEvent) => {
    ...
    if (!formData.title.trim() || !formData.content.trim() || !formData.study_area_id) {
      return
    }
            ...
            {!isRevealed ? (
              ...
              <div className="space-y-4">
                <div className="border-t pt-6">
                  <p className="text-center font-medium mb-4">How difficult was this to remember?</p>
                  <div className="flex justify-center space-x-2">
                    {[1, 2, 3, 4, 5].map((difficulty) => (
                      <button
                        key={difficulty}
                        onClick={() => handleReviewNote(difficulty as 1 | 2 | 3 | 4 | 5)}
    ...
    <div className="space-y-6">
        ...
        <div className="bg-card rounded-lg border p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-orange-600">{dueNotes.length}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">
                {notes.length > 0 
                  ? (notes.reduce((sum, note) => sum + note.difficulty, 0) / notes.length).toFixed(1)
                  : '0'
                }
              </p>
            </div>
      ...
      <div className="flex space-x-1 bg-muted p-1 rounded-lg">
        {(['review', 'all', 'create'] as const).map((tab) => (
          ...
          >
            {tab === 'review' ? `Review (${dueNotes.length})` : 
             tab === 'all' ? `All Notes (${notes.length})` : 
             'Create New'}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          ...
          {dueNotes.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="mt-2">Come back later or create new notes to study.</p>
            </div>
          ) : (
              ...
              {dueNotes.map((note) => (
                <div key={note.id} className="bg-card rounded-lg border p-4">
                  ...
                  <div className="text-xs text-muted-foreground mb-3">
                    ...
                    <p>Difficulty: <span className={getDifficultyColor(note.difficulty)}>
        ...
        <div className="space-y-4">
          ...
          {notes.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p>Create your first study note to start learning with spaced repetition.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {notes.map((note) => (
      ...
      {activeTab === 'create' && (
        <div className="max-w-2xl mx-auto">
          ...
          <form onSubmit={handleCreateNote} className="bg-card rounded-lg border p-6 space-y-4">
            <div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, study_area_id: e.target.value }))}
                className="w-full px-3 py-2 border rounded-md bg-background"
                required
              >
            ...
            <div>
              <label className="text-sm font-medium mb-2 block">Initial Difficulty</label>
